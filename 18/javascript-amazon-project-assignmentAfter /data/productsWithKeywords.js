// 导入 忽略大小写搜索 函数
import { includesIgnoreCase, includesKeywords } from "../scripts/utils/searchKeyWords.js";

// 传入关键词 → 返回true\false
export function searchProduct(productId, searchParams) {
    // 检查 keywods
    for (const product of products) {
        if (product.id === productId) {
            if (includesKeywords(product.keywords, searchParams)) {
                console.log("keywords match at products: ", product);
                return true;
            }
        }
    }
